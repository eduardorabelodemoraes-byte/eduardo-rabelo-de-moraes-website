Warning: truncated output (original token count: 57755)
...
(() => {
  ...
  async function initialize() {
    ...
    try {
      ...
      // Integration Candidate A1 addition (the ONLY change in this file
      // relative to the frozen, approved checkpoint — everything else,
      // including this entire function's control flow, is byte-identical).
      // Purely a plumbing change to WHERE the Home image source comes from;
      // it does not alter uploadTexture()/capturePixelDataDiag() or touch
      // any physics, timing, shader, or choreography code anywhere in this
      // file. When window.__threshold_homeOverride is absent (i.e. this
      // file is loaded exactly as the frozen checkpoint always has been),
      // behavior is identical to the original: loadImage(def.file) fetches
      // the static prebaked PNG.
      const entries = await Promise.all(
        Object.entries(TEXTURES).map(async ([key, def]) => {
          const override = window.__threshold_homeOverride && window.__threshold_homeOverride[key];
      ...
      // Integration Candidate A1 addition (symmetric with the Home seam
      // above, same justification: pure plumbing, no perceptual change).
      // Needed only because this candidate's copy of this file lives one
      // directory level below the document root
      // (threshold-integration/material-engine.js), so GAMES_TEXTURES'
      // hardcoded relative paths ("prebaked/mv-games-*.png", resolved
      // against the DOCUMENT's URL, not this script's own location) 404
      // unless the adapter supplies pre-loaded images from the correct
      // path itself.
      const gamesEntries = await Promise.all(
        Object.entries(GAMES_TEXTURES).map(async ([key, def]) => {
          const override = window.__threshold_gamesOverride && window.__threshold_gamesOverride[key];
          const image = override ? override : await loadImage(def.file);
